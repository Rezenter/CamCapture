...
protected:
            ...
            //-----------------------------------------------------------------------------
    {
        ...
        ReferenceCountedData( Device* pDev, int requestNr ) : m_requestNr( requestNr ), m_imageBufferDesc( 1 ), m_pDev( pDev ),
            m_locator(), m_infoIterator(), m_info_imageProcessingResultsIterator(), m_chunkCounterValues(), m_chunkTimerValues(), m_chunkGains(), m_chunkGainString2IndexMap(), m_refCnt( 1 )
        {
            if( m_requestNr >= 0 )
            {
                std::ostringstream oss;
...
public:
     ...
     *  and then. This parameter allows to configure how often this shall be done.
     *
     *  \note
     *  This property will not be available for every device. Right now only devices operated through the \a GenTL driver package will support
     *  this feature.
    ...
    /**
     *  When working with a GenICam compliant device, GenICam method objects need to be checked for their completion status every now
     *  and then. This parameter allows to configure how often this shall be done at all. A method is checked for completion \c
